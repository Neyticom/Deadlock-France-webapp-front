...
describe("🔐 authMiddleware Tests", () => {
    ...
    beforeAll(() => {
        validToken = jwt.sign({ id: 1, login: "testuser", role: "User" }, process.env.JWT_SECRET as string, { expiresIn: "1h" });
    ...
    it("❌ Devrait refuser l'accès Admin à un utilisateur non Admin", async () => {
        const res = await request(app)
            .get("/admin")
