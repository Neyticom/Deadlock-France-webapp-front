...
const spellEffectController = {
  getAllSpellEffects: async (req: Request, res: Response, next: NextFunction) => {
    try {
      ...
      const spellEffects = await SpellEffect.findAll({ where: { spell_id: spellId } });
  ...
  getSpellEffectById: async (req: Request, res: Response, next: NextFunction) => {
    try {
      const { id: spellId, id: effectId } = req.params;
      const spellEffect = await SpellEffect.findOne({ where: { id: effectId, spell_id: spellId } });
  ...
  createSpellEffect: async (req: Request, res: Response, next: NextFunction) => {
    try {
      const { id: spellId } = req.params;
      const { order, effect } = req.body;

      if (!order || !effect) {
        res.status(400).json({ error: "Missing required fields: order, effect" });
        return;
      }
  ...
  updateSpellEffect: async (req: Request, res: Response, next: NextFunction) => {
    try {
      const { id: spellId, id: effectId } = req.params;
      const spellEffect = await SpellEffect.findOne({ where: { id: effectId, spell_id: spellId } });
      ...
      const updatedSpellEffect = await spellEffect.update(req.body, { fields: Object.keys(req.body) });
  ...
  deleteSpellEffect: async (req: Request, res: Response, next: NextFunction) => {
    try {
      const { id: spellId, id: effectId } = req.params;
      const spellEffect = await SpellEffect.findOne({ where: { id: effectId, spell_id: spellId } });
