...
const itemEffectController = {
  ...
  getAllItemEffects: async (req: Request, res: Response, next: NextFunction) => {
    try {
      const { id: itemId } = req.params;
      const itemEffects = await ItemEffect.findAll({ where: { item_id: itemId } });
  ...
  getItemEffectById: async (req: Request, res: Response, next: NextFunction) => {
    try {
      const { id: itemId, id: effectId } = req.params;
      const itemEffect = await ItemEffect.findOne({ where: { id: effectId, item_id: itemId } });
  ...
  createItemEffect: async (req: Request, res: Response, next: NextFunction) => {
    try {
      const { id: itemId } = req.params;
      const { type, effect } = req.body;

      if (!type || !effect) {
        res.status(400).json({ error: "Missing required fields: type, effect" });
        return;
      }
  ...
  updateItemEffect: async (req: Request, res: Response, next: NextFunction) => {
    try {
      const { id: itemId, id: effectId } = req.params;
      const itemEffect = await ItemEffect.findOne({ where: { id: effectId, item_id: itemId } });
  ...
  deleteItemEffect: async (req: Request, res: Response, next: NextFunction) => {
    try {
      const { id: itemId, id: effectId } = req.params;
      const itemEffect = await ItemEffect.findOne({ where: { id: effectId, item_id: itemId } });
